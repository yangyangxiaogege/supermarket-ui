<template>
    <!--批发退货单-->
    <div id="wholeRollbackList">
        <div class="my-content">
            <!--搜索区-->
            <div class="my-search">
                <el-input v-model="queryOrder.condition" placeholder="客户名称/联系人/联系电话/地址" style="width: 250px" size="small"></el-input>
                <el-button type="primary" size="small" @click="searchOrder">查询</el-button>
            </div>
            <!--工具-->
            <div class="my-tools">
                <el-button icon="glyphicon glyphicon-plus" size="small" @click="openChooseOrder">从批发出库单中生成</el-button>
            ...
            <table class="my-tab table table-bordered">
                ...
                <tr v-for="(item,index) in pageList" :key="item.id.toString()">
                    ...
                    <td v-if="item.takeState == 1">待发货</td>
                ...
                <div>
                    <div class="my-modal modal-content" style="z-index: 2000">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary" @click="submitForm('ruleForm',order.id)">确认退货</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            // 获取采购订单列表
            init(){
                this.$http.post('wholeOrder/findByCondition/','takeState=2').then(result => {
            ...
            searchOrder(){
                ...
                this.$http.post('wholeOrder/findByCondition',params+'&takeState=2').then(result => {
                    this.orderList = result.data;
                    // 获取分页数据
                    this.initPage();
                    this.getPageList(this.currentPage);
            ...
            // 选择要进行发货的订单
            openChooseOrder(){
                this.$http.post('wholeOrder/findByCondition/','singleState=1&takeState=1').then(result => {
            ...
            submitForm(formName,orderId) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        this.$http.post('wholeOrder/inWholeStore',params).then(result => {
